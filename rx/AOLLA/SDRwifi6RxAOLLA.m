    ...
    %带宽决策：带内外子载波SNR均值差距大于10dB缩小带宽
        if chanBW(4) == '4'
          BWdiff = abs(mean([rmsEVMsc(1:117);rmsEVMsc(352:468)])-mean(rmsEVMsc(118:351)));
            if abs(mean([rmsEVMsc(1:117);rmsEVMsc(352:468)])-mean(rmsEVMsc(118:351))) >= 6
            ChannelBWnewDec = 2;
%             chanBW = 'CBW20' ;                %基带带宽
...
[ackarqtxWaveform,MCSnew,ackarqmsdu,PERList,deltaThresholdList] = ackarqtx701LMS(ctlinfo,SNRack,numampdunew,ChannelBWnewDec,chanBW,ctlinfoList,PERList,deltaThresholdList,MCSbuffer);
if ctlinfo ~= 0
    MCSbuffer = MCSnew;
end
ackind = ackind+1;
