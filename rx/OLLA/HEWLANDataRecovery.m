function [packetSeq,rxBitMatrix,rxnumampdu,SNRest,RXTime,pktind,searchOffset,ConstellationDiagram,ctlinfo,rmsEVMsym,rmsEVMsc] = HEWLANDataRecovery(chanBW,sr,rx,idleTime)
%% 功能：WLAN HESU解包
%input:
%chanBW：带宽
%sr：采样率
...
while (searchOffset + minPktLen) <= rxWaveLen
    ...
    timeoffset = wlanSymbolTimingEstimate(searchBufferLLTF,chanBW);
...
if (numRxSamples+pktOffset)>length(rx)
    ...
    rx(pktOffset+(1:numRxSamples+idleTime*sr*0.75),:) = helperFrequencyOffset(rx(pktOffset+(1:numRxSamples+idleTime*sr*0.75),:),sr,-cfoCorrection);
...
%     end
    ...
    if pktind >= 2 
        if rmsind(pktind)>=rmsind(pktind-1)
